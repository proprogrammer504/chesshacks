...
class AlphaZeroPipeline:
    ...
    def __init__(self, checkpoint_dir=None, data_dir=None, log_dir=None, device=None):
        """
        Initialize pipeline
    ...
    def run_iteration(self, iteration, num_self_play_games=None, num_workers=None,
        ...
        print("\n[Step 1/3] Generating self-play games...")
        self_play_start = time.time()
        
        self_play_games = generate_self_play_games_parallel(
            self.model, num_self_play_games, num_workers=num_workers
        )
        
        self_play_time = time.time() - self_play_start
        print(f"Generated {len(self_play_games)} games in {self_play_time:.2f}s")
        ...
        if enable_evaluation and iteration > 0 and iteration % eval_frequency == 0:
            print("\n[Step 3/3] Evaluating against best model...")
            ...
            try:
                ...
                if should_update_best:
                    ...
                    print(f"✗ Best model remains iteration {self.best_model_path}")
            ...
            self.best_model_path = checkpoint_path
            print("\n[Step 3/3] Evaluation skipped (early training)")
        ...
        print(f"\nIteration {iteration} complete in {iteration_time:.2f}s")
    ...
    def run(self, num_iterations, num_self_play_games=None, num_workers=None,
        ...
        print(f"Self-play games per iteration: {num_self_play_games or config.NUM_SELF_PLAY_GAMES}")
...
def main():
    ...
    # Create pipeline
    pipeline = AlphaZeroPipeline(device=args.device)
