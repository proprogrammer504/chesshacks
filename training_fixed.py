...
class FixedGPUBatchedEvaluator:
...
class FixedMCTS:
    """Fixed MCTS with proper move validation"""
    def __init__(self, evaluator, num_simulations=120, c_puct=1.4, 
        ...
        # OPTIMIZATION: Enhanced encoding cache
        self.encoding_cache = {}
        self.encoding_cache_limit = 20000
    ...
    def search(self, board, temperature=1.0):
        ...
        for batch_idx in range(num_batches):
            ...
            # OPTIMIZATION: Progressive pruning
            if (batch_idx + 1) % 10 == 0:
    ...
    def _expand_with_progressive_widening(self, node):
        """Root expansion with proper move validation"""
        board = node.board
    ...
    def _expand_node_with_policy(self, node, policy):
        """Enhanced move ordering with proper validation"""
        board = node.board
        legal_moves = list(board.legal_moves)
        ...
        if node.parent is None:
            ...
            for i, (move, prior) in enumerate(top_moves):
                ...
                if board.is_legal(move):
                    ...
                    node.children[move] = FixedMCTSNode(child_board, parent=node, prior=noisy_prior, move=move)
        else:
            for move, prior in top_moves:
                ...
                if board.is_legal(move):
                    child_board = board.copy()
                    child_board.push(move)
                    node.children[move] = FixedMCTSNode(child_board, parent=node, prior=prior, move=move)
    ...
    def _prune_low_visit_children(self, node):
        """Progressive pruning for performance"""
...
class FixedMCTSNode:
    """Fixed MCTS node with efficient memory layout"""
...
def parallel_self_play_worker_fixed(worker_id, network_state_dict, num_games, result_queue, net_config, device='cpu', move_limit=100):
    """Fixed self-play worker with proper move validation"""
    try:
        ...
        # OPTIMIZATION: Balanced batch size and parallel simulations
        batch_size = 128 if device == 'cuda' else 32
        max_wait_time = 0.002
        evaluator = FixedGPUBatchedEvaluator(network, device=device, batch_size=batch_size, max_wait_time=max_wait_time)
        
        # OPTIMIZATION: Balanced parallel simulations
        parallel_sims = 32 if device == 'cuda' else 8
        mcts = FixedMCTS(evaluator, num_simulations=100, parallel_sims=parallel_sims,
                       top_k_initial=10, prune_threshold=0.08,
                       dirichlet_alpha=0.3, dirichlet_epsilon=0.25)
...
def train_with_fixed_parallel_mcts(iterations=50, games_per_iter=100, num_workers=None,
                                   num_res_blocks=10, num_filters=256):
    """Fixed training function with proper move validation"""
    if num_workers is None:
        num_workers = max(1, mp.cpu_count())
    
    print(f"Training with {num_workers} parallel workers with fixed MCTS")
    ...
    log_file = Path("training_log_fixed.jsonl")
    
    for iteration in range(iterations):
        print(f"\n{'='*60}")
        print(f"FIXED TRAINING ITERATION {iteration + 1}/{iterations}")
        print(f"{'='*60}")
        ...
        # Move limits for balanced games
        move_limit = max(80, 120 - iteration)
        ...
        for worker_id in range(num_workers):
            p = mp.Process(
                target=parallel_self_play_worker_fixed,
        ...
        if (iteration + 1) % 5 == 0:
            checkpoint_path = f"chess_resnet_fixed_iter_{iteration + 1}.pt"
            torch.save({
                'iteration': iteration + 1,
...
if __name__ == "__main__":
    network = train_with_fixed_parallel_mcts(
        iterations=50,
        games_per_iter=100,
        num_workers=max(1, mp.cpu_count()),
        num_res_blocks=10,
    ...
    torch.save(network.state_dict(), "chess_resnet_final_fixed.pt")
    print("\nFIXED Training complete! Final model saved.")